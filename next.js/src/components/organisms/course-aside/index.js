...
const Aside = ({ author, category, complexity, courseLength }) => {
  const socials = [
    {
      name: "Instagram",
  ...
  return (
    <aside className={styles.aside}>
      <div className={styles.author}>
        <Img data={author.img} className={styles.authorImage} />
        <div>
          <p className={styles.title}>автор курса</p>
          <p className={styles.name}>{author.name}</p>
          <p className={styles.specialization}>{author.specialization}</p>
          {socials.length > 0 && (
