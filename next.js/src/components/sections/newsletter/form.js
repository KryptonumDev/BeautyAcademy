    ...
    <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
      ...
      {status.success !== undefined && (
        <div className={styles.state} aria-invalid={!status.success}>
          ...
          <h3 className='h2'>{status.success ? "Спасибо! Вы успешно подписались на нашу рассылку" : "Возникла проблема с подпиской на рассылку новостей"}</h3>
          <p>{status.success ? "Ждите отличных новостей" : "Проверьте правильность указанного адреса электронной почты"}</p>
          {!status.success && (
            <Button
              type="button"
