...
export default function Grid({ slug, products, productCategories }) {
  ...
  const [total, setTotal] = useState(products.pageInfo.total)
  ...
  return (
    <section className={styles.wrapper}>
      ...
      <div className={styles.pagination}>
        <p>{newProducts?.nodes?.length} из {total} курсов</p>
        <span style={{ '--percent': `${(100 / total * newProducts?.nodes?.length) || 100}%` }} className={`${styles.line}`} />
        {newProducts?.nodes?.length !== total && (
